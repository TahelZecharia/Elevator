class Elevator(object):
    ...
    #     self.passengers_in_elevator_list = [None for i in range(len(self.all_calls_list))]

    # This is run once before the simulation begins to set the top_floor taken from the building class
    def set_top_floor(self, max_floor):
    ...
    def new_passenger(self, new_passengers_list):
        ...
        # a dynamic list that contains the identities of people who are waiting
        # outside of the elevator and want to get in
        identities_list_still_waiting = [i.identity for i in new_passengers_list]

        # a static list that contains the identities of all the people who wanted
        # to get into the elevator in the beginning
        ...
        # Adding new passengers, while there are still passengers who want
        # to get on and the elevator still has free space:
        while len(new_passengers_list) > num_passengers_got_on and len(
            ...
            self.passengers_in_elevator_list[id_passenger_gets_on].set_got_src_time(self.cur_time)
        ...
        for element_in_queue_list in self.curr_list:
            ...
            if element_in_queue_list[2] in identities_list_people_who_got_on:
                ...
                indeces_to_delete_from_queue_list.append(self.curr_list.index(element_in_queue_list))
        ...
        for index_to_delete_from_queue_list in range(len(indeces_to_delete_from_queue_list) - 1, -1, -1):
            ...
            del self.curr_list[indeces_to_delete_from_queue_list[index_to_delete_from_queue_list]]
    ...
    def passenger_exit(self, exit_passengers_list):
        ...
        # The destiontions of all passengers that are supposed to exit at
        # this floor should all be this same floor
        exit_destinations = [i.dest for i in exit_passengers_list]
        if exit_destinations != [self.cur_floor for i in range(len(exit_passengers_list))]:
            print
            "Warning!!! Some passenger(s) do(es) not want to exit on this floor!!!"
        else:
            for exit_passenger in exit_passengers_list:
                self.passengers_in_elevator_list[exit_passenger.identity].set_got_dest_time(
    ...
    # This method is used to update the "last_check", which is used to see
    # if people have pressed the elevator button since the last time it was
    # checked. This is important because some actions (people getting on elevator)
    # take time and if the elevator only checks for button presses at the current
    # time, it will have missed those that happened between the last check and current time.
    ...
    def simulation(self):
        ...
        # It then updates the button list, from the action list,
        # adding all actions since it has last been checked
        for passenger in self.all_calls_list:
            if passenger.time > self.last_check and passenger.time <= self.cur_time:
                self.curr_list.append([passenger.src, passenger.type, passenger.identity])
        ...
        if len(self.curr_list) == 0 and self.passengers_in_elevator_list == [None for i in range(len(
                self.passengers_in_elevator_list))]:
        ...
        # In this case, the elevator moves to the most extreme person waiting in its direction of travel.
            ...
            if self.flag == "Up":
                temp_goal = max(item[0] for item in self.curr_list)
                if temp_goal > self.cur_floor:
                    goal = temp_goal
                elif temp_goal < self.cur_floor:
                    self.change_direction()
                    goal = min(item[0] for item in self.curr_list)
                else:
                    goal = self.cur_floor
            elif self.flag == "Down":
                temp_goal = min(item[0] for item in self.curr_list)
                if temp_goal < self.cur_floor:
                    goal = temp_goal
                elif temp_goal > self.cur_floor:
                    self.change_direction()
                    goal = max(item[0] for item in self.curr_list)
                else:
                    goal = self.cur_floor
        ...
        # extreme destination from the people on the elevator.
            ...
            if self.flag == "Up":
                goal = max(
                    passenger.type for passenger in [i for i in self.passengers_in_elevator_list if i != None])
            else:  # self.DOT == "Down"
                goal = min(
                    passenger.type for passenger in [i for i in self.passengers_in_elevator_list if i != None])
        ...
        for i in range(0, len(self.passengers_in_elevator_list)):
            if self.passengers_in_elevator_list[i] != None:
                if self.cur_floor == self.passengers_in_elevator_list[i].type:
                    done_calls.append(self.passengers_in_elevator_list[i])
                    # self.passengers_in_elevator_list[i] = None
                    # It takes 4 seconds for the elevator doors to open and than shut.
                    # People both get on and get off during this time, thus if one passenger is getting on or off, the action will take 4 seconds.
                    self.action_time = 4
        print
        "getting off", [i.identity for i in done_calls]
        self.passenger_exit(done_calls)

        # If a person is on the same level as the elevator and it is moving in their direction of travel, they should get on
        getting_on = []
        for i in range(0, len(self.curr_list)):
            if (self.cur_floor == self.curr_list[i][0] and self.flag == self.curr_list[i][1]):
                getting_on.append(self.all_calls_list[self.curr_list[i][2]])
                self.action_time = 4
        self.new_passenger(getting_on)

        print
        "Queue List is", self.curr_list
        print
        "Current floor =", self.cur_floor
        print
        "Passengers in elevator =", [i.identity for i in self.passengers_in_elevator_list if i != None]
        print
        "Happy people =", self.happy_people
    ...
    #
    #     if self.flag == "Up" and self.cur_floor != self.max_floor:
    #         self.move("Up")
    #     elif self.flag == "Down" and self.cur_floor != 0:
    #         self.move("Down")
    #     elif self.flag == "Up" and self.cur_floor == self.max_floor:
    #         self.change_direction()
    #     elif self.flag == "Down" and self.cur_floor == 0:
    #         self.change_direction()
    #
    ...
    #
    #     print
    #     "getting off", [i.identity for i in getting_off]
    #     print
    #     "Queue List is", self.curr_list
    #     print
    #     "Current floor =", self.cur_floor
...
class Passenger(object):
    ...
    def __init__(self, time=0., dest=1, src=0, identity=0):
        self.src = src
        self.time = time
        self.got_dest_time = 0.
        self.dest = dest
        self.got_src_time = 0.
        self.identity = identity
        self.type = "Up" if (self.dest - self.src) > 0 else "Down"

    def set_got_dest_time(self, time_exited):
        self.got_dest_time = time_exited

    def set_got_src_time(self, pickup_time):
        self.got_src_time = pickup_time

    def get_got_src_time(self):
        return self.got_src_time

    def get_got_dest_time(self):
        return self.got_dest_time
...
#             Passenger(time_appeared=times_of_arrival[i], dest=destinations[i], src=pickup_locations[i],
#                       identity=i))
#
...
#     print
#     "Times appeared:", [i.time_appeared for i in passenger_list]
#     print
#     "Pickup floors:", [i.pickup_floor for i in passenger_list]
#     print
#     "Destinations:", [i.type for i in passenger_list]
#     print
...
# # seconds < br / >
# # Floor
# # Distribution = [110, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10] < br / >
# # Number
# # of
# # Passengers = 20 < br / >
# # This
...
# # with the comparison algorithm.
# #
# # < b > results_matrix_4: This is a
# # large
# # building
# # with lots of floors.< / b > < br / >
# # Number
# # of
# # Floors = 23 < br / >
# # Average
...
# # seconds < br / >
# # Floor
# # Distribution = [220, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10], < br / >
# # Number
# # of
# # Passengers = 20 < br / >
# # This
...
# # with the comparison algorithm.
# #
# # < b > results_matrix_6: This is a
# # building
# # with a skewed distribution.< / b > < br / >
# # Number
# # of
# # Floors = 12 < br / >
# # Average
...
# # seconds < br / >
# # Floor
# # Distribution = [10, 10, 100, 10, 10, 10, 10, 10, 100, 10, 10, 10], < br / >
# # Number
# # of
# # Passengers = 20 < br / >
# # This
...
# # the
# # simulation
# # with the given conditions, and is a list of all passengers and < br / >
# # results_matrix_0[0][0] is the
# # first
# # passenger in the
# # given
# # simulation.
...
#     print
#     "Average time from", name, sum(lis) / float(len(lis))
#     print
#     "Max time from", name, max(lis)
#     print
#     "95th percentile of time from", name, np.percentile(np.array(lis), 95)
#     print
#     "Median of time from", name, np.median(np.array(lis))
#     print
#     "__________\n"
#
#
